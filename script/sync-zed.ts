...
const FORK_REPO = "KSD-CO/zed-extensions"
const UPSTREAM_REPO = "zed-industries/extensions"
const EXTENSION_NAME = "ironcode"
...
async function main() {
  ...
  // Delete any existing branches for ironcode updates
  ...
  const prResult =
    await $`gh pr create --repo ${UPSTREAM_REPO} --base main --head ${FORK_REPO.split("/")[0]}:${branchName} --title "Update ${EXTENSION_NAME} to v${cleanVersion}" --body "Updating IronCode extension to v${cleanVersion}"`
