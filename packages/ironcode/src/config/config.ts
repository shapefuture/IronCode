...
export namespace Config {
  ...
  function getManagedConfigDir(): string {
    switch (process.platform) {
      case "darwin":
        return "/Library/Application Support/ironcode"
      case "win32":
        return path.join(process.env.ProgramData || "C:\\ProgramData", "ironcode")
      default:
        return "/etc/ironcode"
    }
  }
  ...
  export const state = Instance.state(async () => {
    ...
    for (const [key, value] of Object.entries(auth)) {
      if (value.type === "wellknown") {
        ...
        log.debug("fetching remote config", { url: `${key}/.well-known/ironcode` })
        const response = await fetch(`${key}/.well-known/ironcode`)
        if (!response.ok) {
    ...
    const directories = [
      ...
      ...(!Flag.IRONCODE_DISABLE_PROJECT_CONFIG
        ...
        : []),
      // Always scan ~/.ironcode/ (user home directory)
      ...(await Array.fromAsync(
        Filesystem.up({
          targets: [".ironcode"],
  ...
  export async function installDependencies(dir: string) {
    ...
    json.dependencies = {
      ...json.dependencies,
      "@ironcode-ai/plugin": targetVersion,
    }
  ...
  async function needsInstall(dir: string) {
    ...
    const depVersion = dependencies["@ironcode-ai/plugin"]
    if (!depVersion) return true

    const targetVersion = Installation.isLocal() ? "latest" : Installation.VERSION
    if (targetVersion === "latest") {
      const isOutdated = await PackageRegistry.isOutdated("@ironcode-ai/plugin", depVersion, dir)
      if (!isOutdated) return false
      log.info("Cached version is outdated, proceeding with install", {
        pkg: "@ironcode-ai/plugin",
  ...
  async function loadCommand(dir: string) {
    ...
    for await (const item of COMMAND_GLOB.scan({
      ...
      const patterns = ["/.ironcode/command/", "/.ironcode/commands/", "/command/", "/commands/"]
  ...
  async function loadAgent(dir: string) {
    ...
    for await (const item of AGENT_GLOB.scan({
      ...
      const patterns = ["/.ironcode/agent/", "/.ironcode/agents/", "/agent/", "/agents/"]
      const file = rel(item, patterns) ?? path.basename(item)
   ...
   * getPluginName("oh-my-ironcode@2.4.3") // "oh-my-ironcode"
   ...
   * 2. Local ironcode.json
   * 3. Global plugin/ directory
   * 4. Global ironcode.json
   *
  ...
  export function deduplicatePlugins(plugins: string[]): string[] {
    ...
    // e.g., "oh-my-ironcode", "@scope/pkg"
    const seenNames = new Set<string>()

    // uniqueSpecifiers: full plugin specifiers to return
    // e.g., "oh-my-ironcode@2.4.3", "file:///path/to/plugin.js"
  ...
  export const Keybinds = z
    .object({
      ...
      review_toggle: z.string().optional().default("<leader>r").describe("Toggle code review panel"),
      scrollbar_toggle: z.string().optional().default("none").describe("Toggle session scrollbar"),
      username_toggle: z.string().optional().default("none").describe("Toggle username visibility"),
      status_view: z.string().optional().default("<leader>s").describe("View status"),
      git_status: z.string().optional().default("<leader>i").describe("Git source control"),
  ...
  export const Server = z
    .object({
      ...
      mdnsDomain: z.string().optional().describe("Custom domain name for mDNS service (default: ironcode.local)"),
  ...
  export const Info = z
    .object({
      ...
      server: Server.optional().describe("Server configuration for ironcode serve and web commands"),
      command: z
  ...
  export const global = lazy(async () => {
    let result: Info = pipe(
      ...
      mergeDeep(await loadFile(path.join(Global.Path.config, "ironcode.json"))),
      mergeDeep(await loadFile(path.join(Global.Path.config, "ironcode.jsonc"))),
    )
  ...
  function globalConfigFile() {
    const candidates = ["ironcode.jsonc", "ironcode.json", "config.json"].map((file) =>
