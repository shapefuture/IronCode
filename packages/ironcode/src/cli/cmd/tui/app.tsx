...
function App() {
  ...
  createEffect(() => {
    if (!terminalTitleEnabled() || Flag.IRONCODE_DISABLE_TERMINAL_TITLE) return

    if (route.data.type === "home") {
      renderer.setTerminalTitle("IronCode")
      return
    }
  ...
  sdk.event.on(Installation.Event.UpdateAvailable.type, (evt) => {
    toast.show({
      ...
      message: `IronCode v${evt.properties.version} is available. Run 'ironcode upgrade' to update manually.`,
      duration: 10000,
    })
  })
  ...
  return (
    <box
      ...
      onMouseUp={async () => {
        if (Flag.IRONCODE_EXPERIMENTAL_DISABLE_COPY_ON_SELECT) {
          renderer.clearSelection()
          return
        }
...
function ErrorComponent(props: {
  ...
  issueURL.searchParams.set("ironcode-version", Installation.VERSION)
